...
本repo提供三種方式取得壽司郎香港的即時分店及取籌排隊資料。按你的技術程度選擇：

Three ways to use live Sushiro Hong Kong store and SushiPass ticket-queue data
from the free public API — pick the one that matches your skill level.

## 一句即試 Try it in one line (no setup)
...
揀選英文版或香港中文版提示詞，整個 code block 複製，作為第一句訊息貼入任何聊天機械人（ChatGPT、Claude、Gemini 等）即可——無需安裝、無需金鑰，機械人會代你讀取資料。若機械人自己不能讀取網址，它會交給你 `curl` 指令（在 terminal 執行）或網址（貼入瀏覽器開啟）；把輸出貼回對話，機械人就會繼續。
...
**進階 · Advanced — 搭公共交通 Public transport：**
...
注意：交通時間只係估計；排隊資料係 API 即時數據，隨時會變，出發前請先向壽司郎官方渠道核實。Travel times are estimates; queue data is live from the API but changes constantly — verify with official Sushiro channels before heading out.

### 邊條路啱你？
| 使用者類型 | 建議 | 原因 | 所需準備 |
|---|---|---|---|
| 一般用家（無需技術背景） | 下方「一句即試」一節的提示詞 | 直接複製 README 內的提示詞，貼入任何聊天機械人即可，機械人會代你讀取資料 | 只需一個聊天機械人帳號 |
| 進階用家（日常使用 AI 編程助理） | `skill/` | 用 `npx skills add` 一句指令安裝 agent skill，助理會自動按需要讀取及整理資料 | Node.js／npx；或 opencode、Claude Code 等 agent 工具 |
| 專業用戶（自行部署服務） | `mcp/` | 在本機或伺服器運行真正的 MCP 伺服器，接駁任何 MCP 客戶端 | uv 或 Docker |
 
無技術背景，只想要資料 → 用上方「一句即試」一節的內嵌提示詞；進階用家（AI 編程助理）→ 安裝 `skill/` agent skill；專業用戶（自行部署服務）→ 部署 `mcp/` MCP 伺服器。對照詳見本頁開頭的中文摘要表。

### 1. 聊天機械人提示詞（一般用家）

無需安裝任何東西、無需金鑰、無需架設服務——整段提示詞已內嵌於上方[一句即試](#一句即試-try-it-in-one-line-no-setup)一節。

1. 揀選英文版或香港中文版提示詞——兩個 block 互通，效果一樣。
2. 整個 code block 複製，作為**第一句訊息**貼入任何聊天機械人。
3. 若機械人自己不能讀取網址，它會交給你 `curl` 指令（在 terminal 執行）或網址（貼入瀏覽器開啟）；把輸出貼回對話，機械人就會繼續。

機械人會在需要時詢問位置或分店編號，並以你所選提示詞的語言回覆。示例 prompt 見上方[示例 · Example prompts](#示例--example-prompts)一節。

### 2. `skill/` — AI 編程助理（進階用家）

一個獨立的 agent skill（`skill/sushiro-scraper/SKILL.md`），教你的 AI 助理自行讀取壽司郎資料，並附遞補層級（直接 GET → 即棄腳本 → curl → webfetch/websearch），端點被封鎖或搬走時也能自救。

#### 用 skills CLI 安裝（最簡單）

[skills CLI](https://github.com/vercel-labs/skills) 會偵測你已安裝的 agent（opencode、Claude Code、Codex、Cursor 等），並為每個 agent 自動接駁：
...
裝好後直接用自然語言開口問即可。示例 prompt 見上方[示例 · Example prompts](#示例--example-prompts)一節。

助理會按 skill 內的配方讀取資料，有禮貌地抓取（每次呼叫相隔至少 1–2 秒，絕不迴圈），並回傳與 MCP 工具相同的 JSON 結構。

### 3. `mcp/` — 自行部署 MCP 伺服器（專業用戶）

整個專案的源頭：一個 Python MCP SDK v2 伺服器，提供 `list_stores` 及 `get_store_queue` 兩個工具，經 streamable HTTP 提供，並自我節流至每秒最多一個上游請求。
...
將任何 MCP 客戶端指向 `http://127.0.0.1:8000/mcp`。進階設定（環境變數、host/origin allowlist、真實 hostname 部署、GHCR 映像）請看 [`mcp/README.md`](mcp/README.md)。

### 可以取得甚麼資料？

- **分店清單**（`list_stores`）——某位置附近最多 100 間分店，由近至遠：營業狀態、是否派籌、等候組數、地址、地區、座標。
- **分店排隊**（`get_store_queue`）——一間分店的即時取籌情況：即將叫號的籌號（`storeQueue`，第一個元素 = 下一個叫號），並按帳枱 booth／櫃枱 counter 及訂座 reservation 變體分拆。

### 善待 API

上游（`https://sushipass.sushiro.com.hk/api/2.0`）是壽司郎香港實際營運的公開售票系統——免費、無需認證、真實存在。

- 任何兩次呼叫之間至少相隔 1–2 秒，切勿連環輪詢。
- 即時排隊最多每 30–60 秒重查一次，而且只查你真正關心的分店。
- 只讀取公開資料——出現登入牆即停止，不是繞過。

### 非官方聲明、可接受使用及許可證

本專案屬**非官方**教育及研究用途，與壽司郎香港及 SushiPass API 營運者並無任何隸屬或認可關係；「Sushiro／壽司郎／SushiPass」為其權利人的商標，僅用於識別服務。

- 程式碼及文件以 [MIT 許可證](LICENSE) 提供，API 資料權利屬於其擁有人（詳見 [NOTICE.md](NOTICE.md)）。
- 僅供教育及研究用途。使用者須自行遵守適用法律及 API 擁有人的服務條款（詳見 [DISCLAIMER.md](DISCLAIMER.md)：切勿未經授權存取、濫用、過量請求、大量散發抓取或繞過安全管制）。
- 安全或下架事宜：[SECURITY.md](SECURITY.md) ／ jaguarkwokhk@gmail.com。

### 儲存庫結構

`README.md` 內含可複製的聊天機械人提示詞（英文 + 香港中文）；`skill/` 是安裝給 AI 編程助理的 agent skill；`mcp/` 是可自行部署的 MCP 伺服器（Python SDK v2，stdio 或 HTTP，附 Docker 及 CI）。
...
|---|---|---|---|
| No technical background — you just want the data | The prompt in [Try it in one line](#一句即試-try-it-in-one-line-no-setup) above | Copy one block into any chatbot; the chatbot does the fetching for you | Just a chatbot (ChatGPT, Claude, Gemini…) |
| Advanced — you work with AI coding agents | [`skill/`](skill/) | Drop-in agent skill; the agent fetches, parses and self-heals blocked/moved endpoints on its own | An agent such as opencode or Claude Code |
| Professional — you run your own services | [`mcp/`](mcp/) | Self-host the real MCP server and connect any MCP client to it | uv or Docker |

### 1. The chatbot prompt — paste into any chatbot (general users)

Nothing to install, no key, nothing to host — the full prompt is embedded in the [一句即試 · Try it in one line](#一句即試-try-it-in-one-line-no-setup) section above.

1. Pick the English prompt or the 香港中文版提示詞 — both blocks are
   interchangeable and behave the same.
2. Copy the entire code block and send it as your **first message** to any
   chatbot.
3. If the chatbot cannot fetch URLs itself, it will hand you a `curl` command to
   run in a terminal, or a URL to open in your browser — paste the output back
   either way, and the chatbot continues from there.

The chatbot will ask for a location or store id when needed and reply in the
language of the prompt you chose. See the
[示例 · Example prompts](#示例--example-prompts) section above for sample prompts.

### 2. `skill/` — for AI coding agents (advanced users)

A standalone agent skill (`skill/sushiro-scraper/SKILL.md`) that teaches your
agent to fetch Sushiro data itself, with a fallback ladder (direct GET →
throwaway script → curl → webfetch/websearch) so it keeps working when
endpoints are blocked or moved.

#### Install with the skills CLI (easiest)

The [`skills` CLI](https://github.com/vercel-labs/skills) detects which agents
you have installed (opencode, Claude Code, Codex, Cursor, …) and wires the
skill in for each of them:
...
Update later with `npx skills update sushiro-scraper`; remove with
`npx skills remove sushiro-scraper`.
...
Then just ask naturally. See the
[示例 · Example prompts](#示例--example-prompts) section above for sample prompts.

The agent reads the skill's recipes, fetches politely (≥1–2s between calls,
no loops), and returns the same JSON shapes as the MCP tools.

### 3. `mcp/` — self-host the MCP server (professionals)

The source of truth: a Python MCP SDK v2 server exposing two tools —
`list_stores` and `get_store_queue` — over streamable HTTP. It throttles
itself to at most one upstream request per second.
...
Point any MCP client at `http://127.0.0.1:8000/mcp`. For configuration
(env vars, host/origin allowlists, publishing behind a real hostname, the GHCR
image), see [`mcp/README.md`](mcp/README.md).

### What data can you get?

- **Store list** (`list_stores`) — up to 100 branches near a point, closest
  first: opening status, whether the branch is issuing tickets (派籌), number
  of groups waiting, address, area, coordinates.
- **Store queue** (`get_store_queue`) — one branch's live ticket queue: the
  upcoming ticket numbers (`storeQueue`, first element = next ticket served),
  split by booth/counter and reservation variants.

### Be kind to the API

The upstream (`https://sushipass.sushiro.com.hk/api/2.0`) is Sushiro Hong
Kong's official public ticketing system — free, unauthenticated, and real.

- Leave at least 1–2 seconds between calls; never poll in loops.
- Re-check live queues at most once every 30–60 seconds, only for stores you
  actually care about.
- Public data only — a login wall means stop, not bypass.

### Unofficial notice, acceptable use & licence

This is an **unofficial** educational/research project — not affiliated with,
endorsed by, or sponsored by Sushiro Hong Kong (壽司郎) or the SushiPass API
operator. "Sushiro"/"壽司郎"/"SushiPass" are trademarks of their respective
owners, used here only to identify the service.

- Code and docs are released under the [MIT licence](LICENSE); the API data
  itself belongs to its owner — see [NOTICE.md](NOTICE.md).
- Educational and research use only. You are responsible for complying with
  applicable laws and the API owner's terms — see [DISCLAIMER.md](DISCLAIMER.md)
  (acceptable-use rules: no unauthorized access, abuse, excessive traffic,
  scraping fan-out, or circumvention of security controls).
- Security or takedown concerns: [SECURITY.md](SECURITY.md) /
  jaguarkwokhk@gmail.com.

### Repository layout

```
sushiro-ai/
├── README.md # includes the copy-paste chatbot prompt (English + 香港中文)
